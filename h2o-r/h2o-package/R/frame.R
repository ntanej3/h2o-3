...
h2o.target_encode_fit <- function (fr, x, y, fold_column)
{
    ...
    encoding_map <- .eval.driver(.newExpr("target.encoder.fit", fr, x, .quote(y), .quote(fold_column_name)))
    return(encoding_map)
}

h2o.target_encode_transform <- function (encoding_map=NULL, is_train_or_valid=NULL, frame=NULL, x, y, holdout_type=c("kfold", "loo", "none"), blending=NULL, inflection_point=NULL, smoothing=NULL, noise = -1, seed = -1)
{
    # emKeys <- encoding_map
    # emFrameKeys <- encoding_map
    transformed <- .eval.driver(.newExpr("target.encoder.transform", NULL, NULL, frame, x, .quote(holdout_type),
